...
export let DEFAULT_SJCL_PARANOIA = 10;
export let DEFAULT_SJCL_NUMWORDS = 2
export let DEFAULT_SJCL_ITERATIONS = 1000;
export let DEFAULT_SJCL_KEYSIZEBITS = 128;
export let DEFAULT_SJCL_TAGLENGTH = 128;
 ...
 */
export function sjclPbkdf2(password: string, salt: string, iterations: number, keySizeBits: number): string {
 ...
 * returns: hex string
 */
export function sjclRandomHex(numWords: number, paranoia: number) {
 ...
 */
export function sjclCCMencrypt(derivedKey: string, plainText: string, initVector: string, authData: string, tagLenBits: number): string {
 ...
 */
export function sjclCCMdecrypt(derivedKey: string, cipherText: string, initVector: string, authData: string, tagLenBits: number): string {
    // AES pseudorandom function based on derived key
    derivedKey = sjcl.codec.hex.toBits(derivedKey);
    var prf = new sjcl.cipher.aes(derivedKey);
...
interface sjclCrypto {
    sjclPbkdf2(password: any, salt: any, iterations: any, keySizeBits: any): string

    sjclRandomHex(numWords: any, paranoia: any): string

    sjclCCMencrypt(derivedKey: any, plainText: any, initVector: any, authData: any, tagLenBits: any): string

    sjclCCMdecrypt(derivedKey: any, cipherText: any, initVector: any, authData: any, tagLenBits: any): string
}